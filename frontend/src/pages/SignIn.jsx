...
function SignIn() {
  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex 
    justify-center items-center' style={{
      backgroundImage : `url(${bg})`
    }}>
      <form className='w-[90%] h-[725px] max-w-[550px] bg-[#00000062] backdrop-blur
      ...
      onSubmit={handleSignIn}>
        ...
        {err.length > 0 && <p className='text-[red] text-[17px]'>*{err}</p>}
        <button className='min-w-[150px] mt-[30px] h-[60px] bg-white rounded-full
        text-black font-semibold text-[19px]' 
        disabled = {loading}
        ...
        onClick={()=>{
          navigate("/signup")
        }}>Want to create a new account ? <span className='
        text-blue-400'>Sign Up</span></p>
      </form>
    </div>
  )
}
